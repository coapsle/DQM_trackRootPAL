...
% %%%%%%%%%%%%%%%将边界条件中的非零项代入
% BCL=zeros(N^2,5*N^2);
% for i=1:N^2  %赋值（1,1）*(N^2*N^2)矩阵块
%     for j=1:N^2
%         BCL(i,j)=B11*R1(i,j)+B12*1/r(1,ceil(i/N))*LL(i,j)+B44*1/r(1,ceil(i/N))^2*Th1(i,j);
%     end
% end
% for i=1:N^2  %赋值（1,2）*(N^2*N^2)矩阵块
%     for j=1:N^2
%         BCL(i,N^2+j)=B12*1/r(1,ceil(i/N))*Th1(i,j)+B44*(1/r(1,ceil(i/N))*R1(i,j)-1/r(1,ceil(i/N))^2*LL(i,j));
%     end
% end
% for i=1:N^2  %赋值（1,4）*(N^2*N^2)矩阵块
%     for j=1:N^2
%         BCL(i,3*N^2+j)=D11*R1(i,j)+D12*1/r(1,ceil(i/N))*LL(i,j)+D44*1/r(1,ceil(i/N))^2*Th1(i,j);
%     end
% end
% for i=1:N^2  %赋值（1,5）*(N^2*N^2)矩阵块
%     for j=1:N^2
%         BCL(i,4*N^2+j)=D12*1/r(1,ceil(i/N))*Th1(i,j)+D44*(1/r(1,ceil(i/N))*R1(i,j)-1/r(1,ceil(i/N))^2*LL(i,j));
%     end
% end
% %%%%%%%%%%%%%%%将边界条件中的非零项代入
...
for i=1:N %将边界条件中的方程系数phir不等于零的项插入
    for j=1:N^2
        matrixbc1(3*N^2+i,j)=B11*R1(i,j)+B12*1/r(1,ceil(i/N))*LL(i,j)+B44*1/r(1,ceil(i/N))^2*Th1(i,j);
        matrixbc1(3*N^2+i,N^2+j)=B12*1/r(1,ceil(i/N))*Th1(i,j)+B44*(1/r(1,ceil(i/N))*R1(i,j)-1/r(1,ceil(i/N))^2*LL(i,j));
        matrixbc1(3*N^2+i,3*N^2+j)=D11*R1(i,j)+D12*1/r(1,ceil(i/N))*LL(i,j)+D44*1/r(1,ceil(i/N))^2*Th1(i,j);
        matrixbc1(3*N^2+i,4*N^2+j)=D12*1/r(1,ceil(i/N))*Th1(i,j)+D44*(1/r(1,ceil(i/N))*R1(i,j)-1/r(1,ceil(i/N))^2*LL(i,j));
        matrixbc1(3*N^2+(N-1)*N+i,j)=B11*R1((N-1)*N+i,j)+B12*1/r(1,ceil(((N-1)*N+i)/N))*LL((N-1)*N+i,j)+B44*1/r(1,ceil(((N-1)*N+i)/N))^2*Th1((N-1)*N+i,j);
        matrixbc1(3*N^2+(N-1)*N+i,N^2+j)=B12*1/r(1,ceil(((N-1)*N+i)/N))*Th1((N-1)*N+i,j)+B44*(1/r(1,ceil(((N-1)*N+i)/N))*R1((N-1)*N+i,j)-1/r(1,ceil(((N-1)*N+i)/N))^2*LL((N-1)*N+i,j));
        matrixbc1(3*N^2+(N-1)*N+i,3*N^2+j)=D11*R1((N-1)*N+i,j)+D12*1/r(1,ceil(((N-1)*N+i)/N))*LL((N-1)*N+i,j)+D44*1/r(1,ceil(((N-1)*N+i)/N))^2*Th1((N-1)*N+i,j);
        matrixbc1(3*N^2+(N-1)*N+i,4*N^2+j)=D12*1/r(1,ceil(((N-1)*N+i)/N))*Th1((N-1)*N+i,j)+D44*(1/r(1,ceil(((N-1)*N+i)/N))*R1((N-1)*N+i,j)-1/r(1,ceil(((N-1)*N+i)/N))^2*LL((N-1)*N+i,j));
    end
end

% BCL(1:N,1:5*N^2)-matrixbc1(3*N^2+1:3*N^2+N,1:5*N^2)
% BCL((N-1)*N+1:(N-1)*N+N,1:5*N^2)-matrixbc1(3*N^2+(N-1)*N+1:3*N^2+(N-1)*N+N,1:5*N^2)
...
for i=1:N % 将边界条件中等号右边的温度相关项B11*alphaDeltaT+B12*alpha*DeltaT填入方程右边
    matrixbc3(i,1)=0;
    ...
    matrixbc3(3*N^2+i,1)=(B11alpha+B12alpha)*DeltaT;
    matrixbc3(3*N^2+(N-1)*N+i,1)=(B11alpha+B12alpha)*DeltaT;
